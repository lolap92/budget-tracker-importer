...
- **Neu: Das DKB-Gehaltskonto lässt sich anbinden** (Mehr → DKB). Sein Kontostand steht auf der Startseite als eigener Abschnitt unterhalb des Depots – aufgebaut wie die Depot-Kachel, mit Betrag und Stand, und mit Detailseite dahinter. Der DKB-Saldo ist **reine Zusatzinformation** und wird **mit nichts verrechnet**: es ist Geld auf einem anderen Konto. Er fließt in keinen Topf-Saldo, in keinen Kontostand des Cashkontos, in keine Prognose und in keine Minus-Erkennung ein; eine Gesamtsumme über beide Konten gibt es bewusst nicht. Ohne verbundenes Konto erscheint der Abschnitt gar nicht.
- **Angebunden wird über GoCardless Bank Account Data**, einen lizenzierten PSD2-Kontoinformationsdienst – bewusst nicht über FinTS/HBCI: dort müsste der echte Bank-PIN im System liegen, genau das Risiko, das die App bei Trade Republic vermeidet. Hier bleibt statt des Bankzugangs nur ein jederzeit widerrufbarer API-Zugang; die Anmeldung selbst läuft bei der Bank. **Secret-ID und Secret-Key stehen in den Add-on-Optionen, nicht in der Datenbank**, und der Zugriffstoken lebt ausschließlich im Arbeitsspeicher: ihn auf die Platte zu schreiben bräuchte Verschlüsselung, Schlüsselablage und eine Löschstrategie – für etwas, das sich in einer einzigen Anfrage neu holen lässt. Die Freigabe deckt nur Kontostand und Kontostammdaten ab, keine Umsätze und keine Überweisungen.
- **Die Einrichtung kommt ohne Rückleitung aus.** Ein PSD2-Freigabe-Ablauf verlangt eine feste, von außen erreichbare Adresse, die eine Ingress-App nicht hat. Statt darauf zu bauen, wird der Freigabe-Link von Hand geöffnet und der Abschluss über den Stand der Freigabe erkannt – die Seite fragt ihn alle fünf Sekunden selbst ab. Wer eine erreichbare Adresse hat (Nabu Casa oder die lokale HA-Adresse), kann sie als Option `gocardless_redirect_url` nachtragen.
- **Umfasst die Freigabe mehrere Konten, wird nichts geraten:** die Auswahl kommt mit IBAN auf die Seite. Sonst stünde womöglich ein Kreditkartenkonto als „Gehaltskonto" auf der Startseite.
- **Erinnerung vor dem Ablauf:** PSD2-Freigaben gelten höchstens 90 Tage, das lässt sich nicht umgehen. Sieben Tage vorher erscheint „Zugriff läuft in n Tagen ab – erneuern" unter „Zu tun", mit Weg zur Erneuerung.
- **Abgerufen wird sparsam.** Der Kontostand wird nur beim Aufruf der DKB-Seite geholt und nur, wenn der gespeicherte Stand älter als sechs Stunden ist (Add-on-Option `gesamtvermoegen_cache_stunden`; sechs statt vier, weil vier Stunden über den Tag verteilt rechnerisch sechs Abrufe ergeben könnten – das Kontingent des kostenlosen Tarifs liegt üblicherweise bei vier) – sonst steht der gespeicherte Wert mit Zeitstempel da. Die Startseiten-Kachel ruft **nie** selbst ab: sie wird ständig geöffnet und würde das begrenzte Tageskontingent des Dienstes aufbrauchen. Scheitert ein Abruf (Kontingent erschöpft, Freigabe abgelaufen, Dienst nicht erreichbar), bleibt der zuletzt bekannte Stand stehen und der Grund wird daneben genannt.
- Jeder abgerufene Saldo ist ein eigener, unveränderlicher Datensatz mit Zeitstempel – kein Update-in-Place. Der „aktuelle Stand" ist schlicht der neueste Eintrag.
- Test-Suite auf 450 Tests.
